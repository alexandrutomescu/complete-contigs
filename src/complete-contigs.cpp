	...
	)
{
	...
	if ( true )
	{
		for (StaticDigraph::OutArcIt out_arc(graph,node); out_arc != INVALID; ++out_arc)
		{
			...
			if (do_not_optimize_length_2_omnitigs or safe_pairs.count(pair_of_ints(graph.id(in_arc),graph.id(out_arc))) > 0)
			{
				...
				if ((visit.queueSize() == 0) or (visit.start(visitTargets) == INVALID))
				{
					...
					if (no_bad_inbr_reached)
					{
						...
						{
							extend_pair(graph, thread_graph, originalNode2ThreadNode, safe_pairs, out_arc, new_walk, new_internal_nodes_set, new_bad_in_nbrs, collection_of_contigs, reported_arcs, do_not_optimize_length_2_omnitigs);		
						}
					}
				}
			}
	...
	)
{
	...
	for (int i = 0; i < n_safe_pairs; i++)
	{
		...
		int tid = omp_get_thread_num();
		tid++;
    	extend_pair(graph,GET_GRAPH(tid),GET_NODE_MAP(tid), safe_pairs, second_arc, current_walk, internal_nodes_set, bad_in_nbrs, ret, reported_arcs, do_not_optimize_length_2_omnitigs);
	}
